...
const loadCommand = function ({ moduleList, threadID, messageID }) {
 ...
 'tiến hành cài đặt...', 'warn');
    ...
    if (errorList.length != 0) api.sendMessage('[ 𝗖𝗠𝗗 ] - Những lệnh đã xảy ra sự cố khi đang load: ' + errorList.join(' '), threadID, messageID);
    api.sendMessage('[ 𝗖𝗠𝗗 ] - Đã tải thành công ' + (moduleList.length - errorList.length) + ' lệnh ❤️‍🩹', threadID, messageID) 
...
const unloadModule = function ({ moduleList, threadID, messageID }) {
    ...
    return api.sendMessage(`[ 𝗖𝗠𝗗 ] - Đã hủy thành công ${moduleList.length} lệnh ❤️‍🩹`, threadID, messageID);
}
...
module.exports.run = function ({ event, args, api }) {
    ...
    switch (args[0]) {
      ...
      case "-i": {
            ...
            if (!command) return api.sendMessage("[ 𝗖𝗠𝗗 ] - Module bạn nhập không tồn tại!", threadID, messageID);
