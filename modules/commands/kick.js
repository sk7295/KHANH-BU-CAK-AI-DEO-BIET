    ...
    try {
        if (args.join().indexOf('@') !== -1) {
            ...
            for (let o in mention) {
               if (permission.includes(mention[o])) return;
                setTimeout(() => {
        ...
        if (event.type == "message_reply") {
          ...
          if (permission.includes(uid)) return;
                ...
                if (!args[0]) return api.sendMessage(`Vui lòng tag hoặc reply người cần kick`, event.threadID, event.messageID)
                    ...
                    if (args[0] == "all") {
                        ...
                        for (let idUser in listUserID) {
                           if (permission.includes(idUser)) return;
                          setTimeout(() => {
        ...
        return api.sendMessage('cc', event.threadID, event.messageID);
    }
}