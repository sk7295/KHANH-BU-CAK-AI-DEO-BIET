...
const folderimg = __dirname + "/cache/bantx";
    ...
    if (!gameThread) return;
      ...
      if (!gameThread.player.find(i => i.userID == senderID)) return;
       ...
       if (body.toLowerCase() == 'xỉu') {
          ...
          if (vv == vvv) {
            api.sendMessage({body: "𝐂𝐡𝐨̛̀ 𝐁𝐨𝐭 𝐍𝐚̆́𝐜 𝐀̀ 𝐊𝐡𝐨̂𝐧𝐠 𝐂𝐡𝐨̛̀ 𝐁𝐨𝐭 𝐋𝐚̆́𝐜 𝐍𝐡𝐞́ 🎲", attachment: createReadStream(__dirname + "/cache/btx.jpeg")},threadID,async  (err, data)  => { 
...
module.exports.run = async function({ api, event, args, Threads, Users, Currencies }) {
  try {
    ...
    if (args[0] == 'create' || args[0] == 'new' || args[0] == '-c') {
      ...
      if (check == false) return api.sendMessage('⚠ 𝐁𝐚̣𝐧 𝐡𝐢𝐞̣̂𝐧 𝐭𝐡𝐢𝐞̂́𝐮 ' + gameThread.money + '$ 𝐝𝐮̀𝐧𝐠 ?cave 𝐤𝐢𝐞̂́𝐦 𝐭𝐡𝐞̂𝐦 𝐧𝐞̂́𝐮 𝐦𝐮𝐨̂́𝐧 𝐭𝐡𝐚𝐦 𝐠𝐢𝐚 𝐛𝐚̀𝐧 𝐠𝐚𝐦𝐞 😽', threadID, messageID);
