...
module.exports.handleReply = async ({ handleReply, event, api }) => {
  ...
  api.sendMessage(`Thành công ${args[0] == 'add' ? 'chấp nhận' : 'xóa'} lời mời kết bạn thành công ${success.length} người:\n${success.join("\n")}${failed.length > 0 ? `\n» Thất bại với ${failed.length} người: ${failed.join("\n")}` : ""}`, event.threadID, event.messageID);
};
...
module.exports.run = async ({ event, api }) => {
  ...
  for (const user of listRequest) {
    i++;
    msg += (`\n${i}. → Tên: ${user.node.name}`
         + `\n→ Id: ${user.node.id}`
         + `\n→ Url: ${user.node.url.replace("www.facebook", "fb")}`
         + `\n→ Thời gian: ${moment(user.time*1009).tz("Asia/Ho_Chi_Minh").format("DD/MM/YYYY HH:mm:ss")}\n━━━━━━━━━━━━━━━━━━\n`);
  }
  ...
  api.sendMessage({body:`${msg}\n📌 Phản hồi tin nhắn này với nội dung: < add/del > + số thứ tự hoặc < all > nếu muốn thực hiện `, attachment },event.threadID, (e, info) => {
