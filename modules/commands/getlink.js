...
module.exports.run = async ({ api, event, Currencies }) => {
 //event['messageReply']['attachments'][0]['type'] || photo || video || audio
  ...
  const res = await axios.post('https://api-vnh4--dawnlover.repl.co/up', {
  data: oi[u],
  type: oix,
  code: random + u
})
    console.log(u)
    m += `https://api-vnh4--dawnlover.repl.co/view?type=${oix}&code=${random + u}.${t}\n`
  }
