...
const login = require("fca-huydev");
...
//////////////////////////////////////////////
    (async function () {
        while (true) {
          ...
          const nhoang = m.format("HH:mm:ss");
          const hour = m.format("HH")
          if (nhoang == `${hour}:00:00`) {
const res = (await axios.get("https://apinhathoanghaikhkhsau.dawnlover.repl.co/mp40animevideo")).data.url;
            const time = process.uptime();
		 var hours = Math.floor(time / (60 * 60));
		var minutes = Math.floor((time % (60 * 60)) / 60);
	var seconds = Math.floor(time % 60);
            var vnh = (await axios.get("https://apinhathoanghaikhkhsau.dawnlover.repl.co/thinh")).data.url;
            const data = (await axios.get(res, {
              responseType: "stream"
            })).data;
            for (singleID of global.data.allThreadID) {
              loginApiData.sendMessage({
                body:`\nBây giờ là: ${nhoang}\nChúc các bạn 1 ngày vui vẻ\n${vnh}\n\nCảm thấy tin nhắn này phiền thả cảm xúc "😠" để bot gỡ.`,
                attachment: data
              }, singleID)
            };
          };
